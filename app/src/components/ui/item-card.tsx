...
function ItemCard({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="card"
      className={cn(
        'bg-card text-card-foreground hover:border-secondary border-primary-foreground flex flex-col gap-6 rounded-md border py-4 shadow-sm duration-200 hover:cursor-pointer',
        className
      )}
      {...props}
    />
